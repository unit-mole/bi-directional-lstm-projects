...
from .entity_extraction import extract_entities, highlighted_text_html, repair_bio_tags
...
class NERInferencePipeline:
    def __init__(self, model_dir: str | Path | None = None) -> None:
        ...
        with (self.model_dir / "word_to_index.pkl").open("rb") as handle:
            self.word_to_index: dict[str, int] = pickle.load(handle)
        with (self.model_dir / "tag_to_index.pkl").open("rb") as handle:
        ...
        self.lowercase = bool(self.metadata.get("lowercase_tokens", True))
        self.pad_token_id = int(self.metadata.get("pad_token_id", 0))
        self._model = None
    ...
    def _ensure_model_loaded(self) -> None:
        if self._model is not None:
            return
        crf_weights = self.model_dir / self.metadata["true_crf_artifact"]["weights_filename"]
        if crf_weights.exists():
            ...
            model(np.zeros((1, self.max_length), dtype=np.int32), training=False)
            model.load_weights(crf_weights)
            self._model = model
            self._model_kind = "true_bilstm_crf"
            return

        legacy_path = self.model_dir / self.metadata["legacy_artifact"]["filename"]
        if not legacy_path.exists():
            raise FileNotFoundError(
                "No true CRF weights or legacy Keras model were found in the models directory."
            )
        import tensorflow as tf

        self._model = tf.keras.models.load_model(legacy_path, compile=False)
    ...
    def _predict_chunk(self, tokens: list[str]) -> tuple[list[str], list[float], str]:
        self._ensure_model_loaded()
        batch, lengths = prepare_inference_batch(
            [tokens], self.word_to_index, self.max_length, self.lowercase
        )
        ...
        if self._model_kind == "true_bilstm_crf":
            ...
            decoded = viterbi_decode_numpy(emissions, transition_params, lengths)[0]
            probabilities = self._softmax(emissions[0, :length])
            decoder = "trained linear-chain CRF Viterbi"
        else:
            probabilities = np.asarray(self._model.predict(batch, verbose=0))[0, :length]
            if probabilities.ndim != 2:
                raise ValueError("Legacy model returned an unexpected prediction shape.")
            log_emissions = np.log(np.clip(probabilities, 1e-9, 1.0))[None, :, :]
            constraints, start_scores = build_bio_constraints(self.index_to_tag)
            decoded = viterbi_decode_numpy(
                log_emissions, constraints, [length], start_scores=start_scores
            )[0]
            decoder = "BIO-constrained Viterbi over legacy softmax emissions"

        tags = [self.index_to_tag[int(tag_id)] for tag_id in decoded]
        confidences = [round(float(probabilities[i, tag_id]), 4) for i, tag_id in enumerate(decoded)]
    ...
    def predict_text(self, text: str) -> PredictionResult:
        ...
        if not spans:
            raise ValueError("No tokens could be extracted from the input text.")
        tokens = [span.text for span in spans]
        tags: list[str] = []
        confidences: list[float] = []
        decoder = ""
        for _, _, chunk in chunk_sequence(tokens, self.max_length):
            chunk_tags, chunk_confidences, decoder = self._predict_chunk(list(chunk))
        ...
        entities = extract_entities(tokens, tags, confidences, offsets)
    ...
    def predict_batch(self, texts: Iterable[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for document_id, text in enumerate(texts):
            ...
            for entity in result.entities:
                entity_rows.append({"document_id": document_id, **entity})
